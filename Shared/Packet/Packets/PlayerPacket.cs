...
public struct PlayerPacket : IPacket {
    ...
    public void Serialize(Span<byte> data) {
        int offset = 0;
        MemoryMarshal.Write(data[..(offset += Marshal.SizeOf<Vector3>())], in Position);
        MemoryMarshal.Write(data[offset..(offset += Marshal.SizeOf<Quaternion>())], in Rotation);
        AnimationBlendWeights.CopyTo(MemoryMarshal.Cast<byte, float>(data[offset..(offset += 4 * 6)]));
        MemoryMarshal.Write(data[offset++..++offset], in Act);
        MemoryMarshal.Write(data[offset++..++offset], in SubAct);
    }
